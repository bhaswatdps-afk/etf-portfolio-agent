...
async function fetchYahoo(symbol) {
  ...
  try {
    ...
    if (r.ok) { const wrapper = await r.json(); if (wrapper.contents) { const result = extractClose(JSON.parse(wrapper.contents)); if (result) return result; } }
  } catch(_) {}
  try {
    const r2 = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(base2)}`, { signal: AbortSignal.timeout(8000) });
    if (r2.ok) { const wrapper2 = await r2.json(); if (wrapper2.contents) { const result2 = extractClose(JSON.parse(wrapper2.contents)); if (result2) return result2; } }
  } catch(_) {}
  try {
...
async function fetchMarketData() {
  ...
  if (nifty) { market.nifty = nifty.cur; market.niftyPrev = nifty.prev; updateChip('chip-nifty', 'nifty-val', 'nifty-chg', nifty.cur, nifty.prev, '₹', 0); }
  if (midcap) { market.midcap = midcap.cur; market.midcapPrev = midcap.prev; updateChip('chip-midcap', 'mid-val', 'mid-chg', midcap.cur, midcap.prev, '₹', 0); }
  if (vix) {
    market.vix = vix.cur;
  ...
  if (gold) { market.gold = gold.cur; market.goldPrev = gold.prev; updateChip('chip-gold', 'gold-val', 'gold-chg', gold.cur, gold.prev, '$', 0); }
  if (inr) {
    market.inr = inr.cur;
  ...
  renderTriggers();
  checkAndFireAlerts();
  fetchClaudeAdvice(); // refresh Claude advice with new market data (cached 30min)
}
...
function renderTriggers() {
  ...
  TRIGGER_DEFS.forEach(t => {
    ...
    let fired = t.alreadyTriggered, progress = 0, progressColor = '#ffb340', status = 'watching';
    if (t.alreadyTriggered) { status = 'fired'; progress = 100; progressColor = '#00e87a'; }
    else if (metricVal !== null) {
      if (t.direction === 'below') { progress = Math.min(100, Math.max(0, ((t.startRef - metricVal) / (t.startRef - t.threshold)) * 100)); fired = metricVal < t.threshold; }
      else { progress = Math.min(100, (metricVal / t.threshold) * 100); fired = metricVal > t.threshold; }
    ...
    const curDisplay = metricVal !== null ? (t.metric === 'nifty' || t.metric === 'midcap' ? '₹' + Math.round(metricVal).toLocaleString('en-IN') : t.metric === 'vix' ? metricVal.toFixed(1) : t.metric === 'oil' ? '$' + metricVal : t.metric === 'niftyPE' ? metricVal.toFixed(1) + 'x' : metricVal.toFixed(2)) : '—';
    const dist = (metricVal !== null && t.metric === 'nifty' && !t.alreadyTriggered) ? ` · ${(Math.abs((metricVal - t.threshold) / t.threshold) * 100).toFixed(1)}% from trigger` : '';
    ...
    card.innerHTML = `<div class="tc-tranche">${t.tranche}</div><div class="tc-head"><div class="tc-title">${t.label}</div><div class="${badgeClass}">${badge}</div></div><div class="tc-condition">${t.condition}</div><div class="tc-vals"><div class="tc-cur" style="color:${status==='fired'?'var(--green)':status==='alert'?'var(--red)':'var(--text)'}">${curDisplay}</div><div class="tc-cur-lbl">${t.metric==='nifty'?'Nifty 50':t.metric==='vix'?'India VIX':t.metric==='oil'?'Brent crude':t.metric==='niftyPE'?'PE ratio':'—'}</div></div><div class="tc-target-lbl">Threshold: ${t.threshold.toLocaleString('en-IN')}${dist}</div><div class="progress-bar"><div class="progress-fill" style="width:${progress}%;background:${progressColor}"></div></div><div class="tc-action">${t.action}</div><button class="tc-copy" onclick="copyTrigger('${t.id}')">&#8494; Copy action plan</button>`;
    board.appendChild(card);
  });
  const sEl = document.getElementById('s-triggers'), sTxt = document.getElementById('s-trigger-txt');
...
function getMetricVal(metric) {
  if (metric === 'nifty') return market.nifty; if (metric === 'midcap') return market.midcap;
  if (metric === 'vix') return market.vix; if (metric === 'oil') return market.oil;
  if (metric === 'niftyPE') return market.niftyPE; return null;
}
...
function checkAndFireAlerts() {
  TRIGGER_DEFS.forEach(t => {
    ...
    const val = getMetricVal(t.metric); if (val === null) return;
    ...
    if (fired && !triggerStates[t.id + '_fired_today']) {
      ...
      if (Notification && Notification.permission === 'granted') new Notification('ETF Portfolio Agent', { body: t.label + '\n' + t.action.substring(0, 80) + '...' });
    }
  });
...
function renderHoldings() {
  ...
  tbody.innerHTML = ''; let totInv = 0, totVal = 0;
  holdings.forEach((h, i) => {
    const units = (h.invested * 100000) / h.avgNav, curPrice = estimatePrice(h), curVal = (units * curPrice) / 100000;
    const pnl = curVal - h.invested, pct = (pnl / h.invested) * 100;
    ...
    tr.innerHTML = `<td><div class="fund-name">${h.name}</div><div class="fund-ticker">${h.ticker}</div><span class="cat-tag cat-${h.cat}">${catLabels[h.cat]||h.cat.toUpperCase()}</span></td><td class="r">₹${h.avgNav.toFixed(2)}</td><td class="r">${Math.round(units).toLocaleString('en-IN')}</td><td class="r">₹${h.invested.toFixed(2)}L</td><td class="r">₹${curPrice.toFixed(2)}</td><td class="r">₹${curVal.toFixed(2)}L</td><td class="r ${pClass}">${pnl>=0?'+':''}₹${pnl.toFixed(2)}L</td><td class="r ${pClass}">${pct>=0?'+':''}${pct.toFixed(2)}%</td><td><button class="btn-del" onclick="deleteHolding(${i})" title="Remove">×</button></td>`;
    tbody.appendChild(tr);
  });
  const totPnl = totVal - totInv, totPct = (totPnl / totInv) * 100, pc = totPnl >= 0 ? 'pos' : 'neg';
  ...
  document.getElementById('tf-pnl').className = 'r ' + pc; document.getElementById('tf-pnl').textContent = `${totPnl>=0?'+':''}₹${totPnl.toFixed(2)}L`;
  document.getElementById('tf-pct').className = 'r ' + pc; document.getElementById('tf-pct').textContent = `${totPct>=0?'+':''}${totPct.toFixed(2)}%`;
...
function addHolding() {
  ...
  if (idx >= 0) {
    const h = holdings[idx]; const oldUnits = (h.invested * 100000) / h.avgNav; const newUnits = (amt * 100000) / nav;
    h.invested += amt; h.avgNav = (h.invested * 100000) / (oldUnits + newUnits);
...
function deleteHolding(i) { if (!confirm(`Remove ${holdings[i].name}?`)) return; holdings.splice(i, 1); localStorage.setItem('etf_holdings', JSON.stringify(holdings)); renderHoldings(); showToast('Removed'); }
function toggleAddPanel() { const p = document.getElementById('add-panel'); p.style.display = p.style.display === 'none' ? 'block' : 'none'; }

// ─── APPS SCRIPT ──────────────────────────────────────────────────────────────
function copyScript() { navigator.clipboard.writeText(APPS_SCRIPT_CODE).then(() => showToast('Apps Script code copied — paste into script.google.com')); }
function copyTrigger(id) { const t = TRIGGER_DEFS.find(x => x.id === id); if (!t) return; navigator.clipboard.writeText(`${t.label}\nCondition: ${t.condition}\nAction: ${t.action}`).then(() => showToast('Action plan copied')); }
...
function saveGasUrl() {
  ...
  if (!url.startsWith('https://script.google.com/macros/')) { showToast('Please enter a valid Apps Script Web App URL'); return; }
...
async function fetchClaudeAdvice(force = false) {
  ...
  if (!gasUrl) { document.getElementById('claude-setup-notice').style.display = 'flex'; document.getElementById('claude-loading').style.display = 'none'; return; }
  const now = Date.now();
  ...
  try {
    const payload = {
      market: { nifty: market.nifty, niftyPrev: market.niftyPrev, midcap: market.midcap, vix: market.vix, gold: market.gold, inr: market.inr, oil: market.oil, niftyPE: market.niftyPE },
    ...
    if (data.news && data.news.length > 0) renderNews(data.news);
    ...
    recsEl.innerHTML = '<div class="claude-error">⚠ Failed: ' + err.message + '. Check your Apps Script Web App URL.</div>';
  } finally {
...
function renderNews(newsItems) {
  ...
  if (!newsEl) return;
  newsEl.innerHTML = newsItems.map((item, i) => '<div class="news-item"><span class="news-item-num">' + (i+1) + '.</span><span>' + item + '</span></div>').join('');
}
...
function renderClaudeRecs(text) {
  ...
  if (blocks.length === 0) { recsEl.innerHTML = '<div class="claude-error">Could not parse recommendations.</div>'; return; }
  let html = '';
  blocks.forEach((block, i) => {
    ...
    html += '<div class="rec-urgency-' + urgency + '"><div class="claude-rec-card"><div class="rec-num">0' + (i+1) + '</div><div class="rec-body"><div class="rec-action">' + action + '</div>' + (reason ? '<div class="rec-reason">' + reason + '</div>' : '') + '</div><div class="rec-urgency-badge">' + urgency + '</div></div></div>';
  });
  html += '<div class="claude-footer"><div class="claude-footer-dot"></div>Claude Haiku &middot; Live Nifty, VIX, Gold + market news &middot; Not financial advice</div>';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderHoldings();
  renderTriggers();
  fetchMarketData(); // also triggers fetchClaudeAdvice internally
